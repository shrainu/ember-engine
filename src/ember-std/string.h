...
static String STRING_TRIM_WHITESPACE(String* string) {
    ...
    for (int32_t i = 0; i < string->lenght; ++i) {

        char crnt = string->buffer[i];
        ...
        } else if (in && (crnt == ' ' || crnt == '\t')) {
            in = false;
        }

        if (in) {
            STRING_PUSH(&trimmed, crnt);
        }
    }
